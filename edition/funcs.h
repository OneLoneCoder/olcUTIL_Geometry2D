...
struct SdfBox {
	...
	inline Box3i get_box() {
		return Box3i::from_min_max(begin, end);
	}
};
	...
	void operator()() {
		...
		if (channel == VoxelBufferInternal::CHANNEL_SDF) {
			switch (mode) {
				case MODE_ADD: {
					// TODO Support other depths, format should be accessible from the volume
					SdfOperation16bit<SdfUnion, SdfBox> op;
					op.shape = shape;
					op.op.strength = strength;
					blocks.write_box(box, VoxelBufferInternal::CHANNEL_SDF, op);
				} break;
				...
				case MODE_REMOVE: {
					SdfOperation16bit<SdfSubtract, SdfBox> op;
					op.shape = shape;
					op.op.strength = strength;
					blocks.write_box(box, VoxelBufferInternal::CHANNEL_SDF, op);
				} break;
				...
				case MODE_SET: {
					SdfOperation16bit<SdfSet, SdfBox> op;
					op.shape = shape;
					op.op.strength = strength;
					blocks.write_box(box, VoxelBufferInternal::CHANNEL_SDF, op);
				} break;
			...
			BlockySetOperation<uint32_t, SdfBox> op;
			op.shape = shape;
			op.value = blocky_value;
			blocks.write_box(box, channel, op);
		}
